...
with row1_col2:
    ...
    fig_profit_rate = px.line(
        df_profit_rate_plot,
        x='x', y='수익률',
        markers=True, text='수익률'
    )
    fig_profit_rate.update_traces(
        mode='lines+markers',
        line=dict(width=3, color='#687E8E'),
        marker=dict(size=8, color='#964F4C', line=dict(width=1, color='#333')),
    ...
    fig_profit_rate.update_layout(
        ...
        showlegend=False,
        xaxis=dict(
            type='linear', range=[-0.1, 1.1],
            tickmode='array', tickvals=[0, 1], ticktext=['현재', '시뮬레이션']
        )
    )
